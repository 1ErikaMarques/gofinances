...
export function Dashboard() {
  ...
  function getLastTansactionDate(
    collection: DataListProps[],//tipando
    type: 'positive' | 'negative') 
    {
      //calculo ultima transacao
    const lastTransaction = new Date(
      Math.max.apply(Math, collection //o math ira cuidar para descobrir o maior numero      
        .filter(trasaction => trasaction.type === type)//tipo da transaction e DatalistProps,pego cada transacao positive
  ...
  async function loadTransactions() {
    ...
    const lastTransactionOExpensives = getLastTansactionDate(transactions, 'negative');
    const totalInterval = `01 a ${lastTransactionOExpensives}`
    ...
    setHighlightData({      
      ...
      expensives: {
        ...
        lastTransaction: `Ultima saida dia ${lastTransactionOExpensives}`,
      },
       total: {
  ...
  return (
    <Container>
      ...
      <>
        ...
        <UserWrapper>
            <UserInfo>
              <Photo source={{ uri: 'https://avatars.githubusercontent.com/u/4424108?v=4'}}/>
              <User>
                <UserGreeting>Ola,</UserGreeting>
                <UserName>Juanzinho</UserName>
              </User>
            </UserInfo>
